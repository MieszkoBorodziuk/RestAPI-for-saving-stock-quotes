...
2 createCompany.sh - send POST request <host>/company with body {"symbol": "<symbol>"}. 

Run:```./createCompany.sh <host> <symbol>```.
...
4 concurrencyTest.sh - run createCompany.sh for (ADA) and (KTK) company, then simultaneously run createStockQuotes.sh 2 times for each company and at the end wait 5sec and run get.sh to check 
if return stock quotes length is 1. 

 Run:```./concurrencyTest.sh <url>```.

 example: ```./concurrencyTest.sh localhost3000```
